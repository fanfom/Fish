...
def get_auk(Arduino=serial.Serial()):
    ...
    time.sleep(1)
    Arduino.write(("LClick{" + str(Mouse.position[0]) + "|" + str(Mouse.position[1]) + "}[" + "270" + ",180]").encode())
    wait_arduino(Arduino)
    time.sleep(1)
    Arduino.write(("LClick{" + str(Mouse.position[0]) + "|" + str(Mouse.position[1]) + "}[" + "840 " + ",735]").encode())
    wait_arduino(Arduino)
    time.sleep(1)
    ...
    for i in range(len(contours) - 1, 0, -1):
        ...
        if str(imagehash.phash(Image.fromarray(roi))) == "bfd0a5cad5aad081":
            if (n % 8) != 0:
                s = "Loot{" + str(Mouse.position[0]) + "|" + str(Mouse.position[1]) + "}[" + str(
                    int(995 + (n % 8 * 57))) + "," + str(350 + int((n) / 8) * 50) + "]"
                Arduino.write(s.encode())
                wait_arduino(Arduino)
                print(s, n)
                ...
                s = "Loot{" + str(Mouse.position[0]) + "|" + str(Mouse.position[1]) + "}[" + str(
                    int(995 + (n % 8 * 60))) + "," + str(350 + int((n) / 8) * 50) + "]"
                Arduino.write(s.encode())
                wait_arduino(Arduino)
                print(s, n)
...
n = 0
while True:
    # if (get_active_window()[:12]=="Black Desert")and
    if mode == "Gar":
        # start_time = timer()
        # t=timer()
        ...
        if (fish > 3000):
            screen_fishing = cv2.cvtColor(
                array(grab.grab({"top": 330, "left": 610, "width": 690, "height": 420}), uint8), cv2.COLOR_RGB2BGR)
            ...
            if fishx != 0 and targetx != 0:
                ...
                Arduino.write("space".encode())
                wait_arduino(Arduino)
                s = "Move{" + str(fishx) + "|" + str(fishy) + "}[" + str(320 + random.random() * 100) + "," + str(
        ...
        getloot(Arduino)
        time.sleep(1)
    # (get_active_window()[:12] == "Black Desert") and
    if (mode == "Rod") or (mode == "Gold") or (mode == "Start"):
        # Ожидание рыбы
        inwater = False
